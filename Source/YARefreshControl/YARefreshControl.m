...
//  YAUIKit
//
//  Created by Candyan on 13-10-18.
...
{
    if (self.refreshingDirection == direction) {
        return;
    }
    ...
    switch (direction) {
        case kYARefreshDirectionTop:
            ...
            contentInset = UIEdgeInsetsMake(refreshingInset + contentInset.top, contentInset.left, contentInset.bottom,
                                            contentInset.right);
            contentOffset = CGPointMake(0, -contentInset.top);
            break;
        case kYARefreshDirectionLeft:
            refreshableDirection = kYARefreshableDirectionLeft;
            contentInset = UIEdgeInsetsMake(contentInset.top, refreshingInset + contentInset.left, contentInset.bottom,
                                            contentInset.right);
        ...
        case kYARefreshDirectionBottom:
            ...
            contentOffset =
                CGPointMake(0, _scrollView.contentSize.height - _scrollView.bounds.size.height + contentInset.bottom);
            break;
        case kYARefreshDirectionRight:
            refreshableDirection = kYARefreshableDirectionRight;
            contentInset = UIEdgeInsetsMake(contentInset.top, contentInset.left, contentInset.bottom,
...
- (void)_checkOffsetsForDirection:(YARefreshDirection)direction change:(NSDictionary *)change
{
    ...
    switch (direction) {
        ...
        case kYARefreshDirectionBottom: {
            ...
            contentInset = UIEdgeInsetsMake(contentInset.top, contentInset.left,
                                            (contentInset.bottom + refreshableInset), contentInset.right);
            break;
        }
        case kYARefreshDirectionRight:
            refreshableDirection = kYARefreshableDirectionRight;
    ...
    if (!(self.refreshingDirection & refreshingDirection)) {
        ...
        if (canEngage) {
            ...
            if (_wasDragging != _scrollView.dragging && _scrollView.decelerating &&
    ...
    if (_canRefreshDirection & direction) {
        ...
        switch (direction) {
            ...
            case kYARefreshDirectionBottom:
                ...
                originY += self.originContentInsets.bottom;
                break;
