...
- (void)refreshControl:(YARefreshControl *)refreshControl
didShowRefreshViewHeight:(CGFloat)progress
           atDirection:(YARefreshDirection)direction;

- (void)refreshControl:(YARefreshControl *)refreshControl
didRefreshStateChanged:(YARefreshState)refreshState
           atDirection:(YARefreshDirection)direction;

/*
 * inset threshold to engage refresh
 */
- (CGFloat)refreshControl:(YARefreshControl *)refreshController
refreshableInsetForDirection:(YARefreshDirection)direction;

/*
 * inset that the direction retracts back to after refresh started
 */
- (CGFloat)refreshControl:(YARefreshControl *)refreshController
refreshingInsetForDirection:(YARefreshDirection)direction;
